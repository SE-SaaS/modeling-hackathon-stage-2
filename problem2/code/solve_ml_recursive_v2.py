...
def extract_features(df):
    ...
    # Holiday anomaly features (December specifically causes the most chaos in time series)
    df['is_xmas'] = ((df['month'] == 12) & (df['dayofmonth'].isin([24, 25, 26, 31]))).astype(int)
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(base_dir, 'output', 'submission_ml_recursive_v2.csv')
    ...
    print("Generating dense Lag Features (1h, 2h, 3h, 24h, 48h, 168h, 336h, 8736h)...")
    train_full['lag_1'] = train_full['Demand_MW'].shift(1)
    train_full['lag_2'] = train_full['Demand_MW'].shift(2)
    train_full['lag_3'] = train_full['Demand_MW'].shift(3)
    train_full['lag_24'] = train_full['Demand_MW'].shift(24)
    train_full['lag_48'] = train_full['Demand_MW'].shift(48)
    train_full['lag_168'] = train_full['Demand_MW'].shift(168)
    train_full['lag_336'] = train_full['Demand_MW'].shift(336)
    ...
    features = [
        'Hour', 'dayofweek', 'dayofmonth', 'month', 'dayofyear', 'is_weekend', 'is_xmas',
        'hour_sin', 'hour_cos', 'day_sin', 'day_cos', 'month_sin', 'month_cos',
        'lag_1', 'lag_2', 'lag_3', 'lag_24', 'lag_48', 'lag_168', 'lag_336', 'lag_8736'
    ]
    ...
    for i in range(len(test_extracted)):
        ...
        row['lag_1'] = history_demand[-1]
        row['lag_2'] = history_demand[-2]
        row['lag_3'] = history_demand[-3]
        row['lag_24'] = history_demand[-24]
        row['lag_48'] = history_demand[-48]
        row['lag_168'] = history_demand[-168]
        row['lag_336'] = history_demand[-336]
        ...
        # INJECT PREDICTION INTO HISTORY for the next recursive step
