...
app = modal.App("problem2-dl-ensemble")
...
def train_and_predict(train_csv_bytes: bytes, test_csv_bytes: bytes):
    ...
    models = [
        NHITS(h=horizon, input_size=168, max_steps=500), # 1 week input (168 hours)
        PatchTST(h=horizon, input_size=168, max_steps=500),
        LSTM(h=horizon, input_size=168, max_steps=500)
    ]
    
    nf = NeuralForecast(models=models, freq='h')
    
    logging.info("Training Models on A10G Cloud GPU (this will take a few minutes)...")
...
def main():
    ...
    print("Uploading data and starting NeuralForecast Ensemble on Modal GPU...")
    print("This usually takes about 5-10 minutes. Check your Modal dashboard for real-time logs!")
